    ...
    <div class="portlet-box">
        <div class="portlet-header flex-row flex d-flex align-items-center">
            <div class="flex d-flex flex-column">
              <?php $this->load->view('Survey/_SearchCriteria');?>
            </div>
        </div>
        <div class="portlet-body no-padding" id="tableData">
            <?php $this->load->view('Survey/_Gridview');?>
        </div>
    </div>
    <div class="modalAddEdit">
    <?php $this->load->view('Survey/_ModalAddEdit');?>
    </div>
</div>
...
<script type="text/javascript">
        ...
        function onSearchCriteria(page) {
            ...
            $.ajax({
                ...
                url: "Survey/Search/",
            data: {
                SurveyCd: $("#txtSearchSurveyCd").val(),
                SurveyName: $("#txtSearchName").val(),
                currentPage : page
            },
        ...
        function clearAddEdit(data) {
            ...
            $("#txtAddEditSurveyCd").val('');
            $("#txtAddEditSurveyName").val('');
            $("#txtAddEditPrice").val('');
            $("#txtAddEditSurveyDesc").val('');
            $("#txtAddEditSurveyCd").prop('disabled', false);
            $("#btnSubmit").attr('onclick', 'btnSubmitEdit_onClick("ADD");');
            $("#btnSubmit").show();
        }
        else if (data == "EDIT") {
            $("#txtAddEditSurveyCd").prop('disabled', true);
        ...
        function btnEdit_OnClick(SurveyCd) {
            setScreenToEditMode();
    ...
    function onEditGetDataSuccess(obj) {
        var data = obj.Survey;
        $("#txtAddEditSurveyCd").val(data.Survey_CD);
        $("#txtAddEditSurveyName").val(data.Survey_NAME);
        $("#txtAddEditSurveyDesc").val(data.Survey_DESC);
        ...
        function btnSubmitEdit_onClick(scrnmd) {
            ...
            if ($("#txtAddEditTitle").val() == "")
                ...
                success: function (returnResult) {
                    ...
                    {
                        swal.close();
                        successMsg("Survey", "Survey berhasil ditambahkan/diupdate");
        ...
        function btnDelete_OnClick(SurveyCd) {
                var params = new Object();
                params.APPLICANT_ID = SurveyCd;
                $.ajax({
                    type: "POST",
                    url: "",
                    contentType: 'application/json',
                    ...
                    success: function (returnResult) {
                        if (returnResult.Result == 'ERROR') {
                            ...
                            successMsg("Delete", "Survey berhasil dihapus");
                        }
                    },
                    error: function (returnResult) {
                        alert('Action error');
